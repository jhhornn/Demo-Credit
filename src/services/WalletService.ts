...
class WalletService {
  ...
  public async transfer(fromWalletId: string, toWalletId: string, amount: number): Promise<void> {
    await this.knex.transaction(async (trx) => {
      const toWallet = await this.walletModel.getWalletByWalletId(toWalletId)
      if (!toWallet) {
        throw new customError.BadRequestError('Recipient\'s wallet ID does not exist');
      }
      const fromWallet = await this.walletModel.getWalletByWalletId(fromWalletId);
